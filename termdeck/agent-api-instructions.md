...
Returns session_id; execution is asynchronous. GET /api/sessions/{session_id}/last-turns
for what it answered and whether it is still processing (?limit=N counts answers,
?final=true keeps only finished ones); /status has the full session state. Follow up via

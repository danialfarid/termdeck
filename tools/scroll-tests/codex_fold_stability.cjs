...
(async () => {
  const response = await fetch(`${BASE}/api/sessions`, {
    ...
    body: JSON.stringify({ model: 'none', permission: 'default', cwd: '/Users/dan/workspace/termdeck',
      title: 'codex-fold-stability' }),
  });
  const id = (await response.json()).session_id;
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage({ viewport: { width: 1400, height: 850 } });
  await page.goto(`${BASE}/p/termdeck`, { waitUntil: 'domcontentloaded' });
